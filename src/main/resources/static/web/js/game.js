...
function refreshGameView(_url) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#gameStateBlock").html(
        '<span class="gameStateLabel">TURN: </span><span class="gameStateLabelBig">' +
      ...
      if (gamePlayerData.gameState == "PLAY") {
        ...
        $("#postSalvo").click(function () {
          ...
          if (salvoPositions.length === 0) {
            $("#errorSalvo").text(
              "Error! No salvos to fire! You must set at least one target!"
            );
...
function showSelf(gamePlayerData) {
  ...
  $("#gamePlayerDetails").html(
    '<span class="labelGame">Game ID: </span><span class="labelGameBig">' +
      gamePlayerData.id +
      '</span><span class="labelGame"> Created: </span><span class="labelGameBig">' +
      DateCreated +
      "</span>"
  );
  ...
  gamePlayerData.ships.forEach(function (ship) {
    ...
    if (ship.locations[0].substring(1) === ship.locations[1].substring(1)) {
      $(firstCellID).html(
        '<img class="shipsImgOnSelfGridVer" src="img/' + ship.type + 'ver.png">'
      );
    } else {
      $(firstCellID).html(
        '<img class="shipsImgOnSelfGridHor" src="img/' + ship.type + 'hor.png">'
      );
    }
  ...
  for (let i = 0; i <= rows; i++) {
    ...
    if (i == 0) {
      ...
      for (let j = 0; j < cols; j++) {
        ...
        $("<td></td>")
          .attr("id", player + String.fromCharCode(65 + (i - 1)) + "" + (j + 1))
          .appendTo(row);
      }
    }
  }
...
function displayOverlay(text) {
  $(
    "<table id='overlay'><tbody><tr><td>" +
      text +
      "<br><button class='btn btn-info' onclick='removeOverlay()'>Ok! I got it.</button> </td></tr></tbody></table>"
  )
    .css({
      position: "absolute",
      margin: "auto",
...
function removeOverlay() {
  $("#overlay").hide("puff", "slow", function () {
    $("#overlay").remove();
  });
}
...
function makeSalvoJSON() {
  ...
  if (
    ...
    salvo1cellID !== "salvoout2" &&
    salvo1cellID !== "salvoout3" &&
    salvo1cellID !== "salvoout4" &&
    salvo1cellID !== "salvoout5"
  ) {
    salvoPositions.push(salvo1cellID);
  }
  if (
    salvo2cellID !== "salvoout1" &&
    salvo2cellID !== "salvoout2" &&
    salvo2cellID !== "salvoout3" &&
    salvo2cellID !== "salvoout4" &&
    salvo2cellID !== "salvoout5"
  ) {
    salvoPositions.push(salvo2cellID);
  }
  if (
    salvo3cellID !== "salvoout1" &&
    salvo3cellID !== "salvoout2" &&
    salvo3cellID !== "salvoout3" &&
    salvo3cellID !== "salvoout4" &&
    salvo3cellID !== "salvoout5"
  ) {
    salvoPositions.push(salvo3cellID);
  }
  if (
    salvo4cellID !== "salvoout1" &&
    salvo4cellID !== "salvoout2" &&
    salvo4cellID !== "salvoout3" &&
    salvo4cellID !== "salvoout4" &&
    salvo4cellID !== "salvoout5"
  ) {
    salvoPositions.push(salvo4cellID);
  }
  if (
    salvo5cellID !== "salvoout1" &&
    salvo5cellID !== "salvoout2" &&
    salvo5cellID !== "salvoout3" &&
    salvo5cellID !== "salvoout4" &&
...
function makeGameRecordTable(hitsArray, gameRecordTableId) {
  ...
  hitsArray.forEach(function (playTurn) {
    ...
    if (playTurn.damages.carrierHits > 0) {
      hitsReport +=
        "Carrier " + addDamagesIcons(playTurn.damages.carrierHits, "hit") + " ";
    ...
    if (playTurn.damages.battleshipHits > 0) {
      hitsReport +=
        "Battleship " +
        addDamagesIcons(playTurn.damages.battleshipHits, "hit") +
        " ";
      if (playTurn.damages.battleship === 4) {
        hitsReport += "SUNK! ";
        $(playerTag + "battleshipIcon").html(
          '<img src="img/battleshipsunk.png">'
        );
        shipsAfloat--;
      }
    }
    if (playTurn.damages.submarineHits > 0) {
      hitsReport +=
        "Submarine " +
        addDamagesIcons(playTurn.damages.submarineHits, "hit") +
        " ";
      if (playTurn.damages.submarine === 3) {
        hitsReport += "SUNK! ";
        $(playerTag + "submarineIcon").html(
          '<img src="img/submarinesunk.png">'
        );
        shipsAfloat--;
      }
    }
    if (playTurn.damages.destroyerHits > 0) {
      hitsReport +=
        "Destroyer " +
        addDamagesIcons(playTurn.damages.destroyerHits, "hit") +
        " ";
    ...
    if (playTurn.damages.patrolboatHits > 0) {
      hitsReport +=
        "Patrol Boat " +
        addDamagesIcons(playTurn.damages.patrolboatHits, "hit") +
        " ";
      if (playTurn.damages.patrolboat === 2) {
        hitsReport += "SUNK! ";
        $(playerTag + "patrolboatIcon").html(
          '<img src="img/patrolboatsunk.png">'
        );
        shipsAfloat--;
      }
    }
    ...
    if (playTurn.missed > 0) {
      hitsReport +=
        "Missed shots " + addDamagesIcons(playTurn.missed, "missed") + " ";
    }
