  ...
  for (var i = 0; i < gamesData.length; i++) {
    ...
    for (var j = 0; j < gamesData[i].gamePlayers.length; j++) {
      if (gamesData[i].gamePlayers.length == 2) {
        $("<td>" + gamesData[i].gamePlayers[j].player.email + "</td>").appendTo(
          row
        );
      }
      if (
        gamesData[i].gamePlayers.length == 1 &&
        (data.player == "Guest" ||
          data.player.id == gamesData[i].gamePlayers[j].player.id)
      ) {
        $(
          "<td>" +
            gamesData[i].gamePlayers[0].player.email +
            '</td><td class="yellow500">WAITING FOR PLAYER</td>'
        ).appendTo(row);
      }
      if (
        gamesData[i].gamePlayers.length == 1 &&
        ...
        joinButtonHtml =
          '<td class="textCenter"><button class="joinGameButton btn btn-info" data-gameid=' +
          '"' +
          gamesData[i].id +
          '"' +
          ">JOIN GAME</button></td>";
    ...
    if (isLoggedPlayer === true) {
      ...
      $(
        '<td class="textCenter"><a href=' +
          ...
          gameUrl +
          '"' +
          'class="btn btn-warning" role="button">ENTER GAME</a></td>'
      ).appendTo(row);
...
function showScoreBoard(playersArray) {
  ...
  for (var m = 0; m < playersArray.length; m++) {
    ...
    if (playersArray[m].scores.length > 0) {
      ...
      $(
        "<td class='textCenter'>" + playersArray[m].total.toFixed(1) + "</td>"
      ).appendTo(row);
